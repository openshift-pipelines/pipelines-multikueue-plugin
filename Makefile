...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=controller-role paths="./..."
...
# architectures. (i.e. make build IMG=myregistry/mypoperator:0.0.1). To use this option you need to:
...
.PHONY: build
build: ## Build and push docker image for the manager for cross-platform support
	# copy existing Dockerfile and insert --platform=${BUILDPLATFORM} into Dockerfile.cross, and preserve the original Dockerfile
	sed -e '1 s/\(^FROM\)/FROM --platform=\$$\{BUILDPLATFORM\}/; t' -e ' 1,// s//FROM --platform=\$$\{BUILDPLATFORM\}/' Dockerfile > Dockerfile.cross
	- docker buildx build --push --platform=$(PLATFORMS) --tag ${IMG} --provenance=false --label quay.expires-after=1d -f Dockerfile.cross .
...
apply: build release
	...
	$(KUBECTL) wait --for=condition=Available deployment pipelines-multikueue-plugin-controller -n tekton-kueue --timeout=300s
